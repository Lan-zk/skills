...
本文档是 Skill 开发的权威规范，完全自包含，可独立迁移至任意 Skill 仓库。

**使用方式：**
- 拷贝至任意 Skill 仓库根目录或 `.claude/` 目录下
- AI 在构建或修改 Skill 时必须遵循本文档
- 人在开发 Skill 时以此为第一设计依据
- 违反本文档中的【强制】项将导致 Skill 不可接受
...
│  【禁止】全流程脚本化 / LLM 逻辑写入 scripts/ / 无条件分支的脚本链  │
├──────────────────────────────────────────────────────────────────┤
│  references/                                                      │
│  知识固化层                                                        │
...
|------|----------------|----------------|
| **SKILL.md** | 触发路由；非确定性决策（分析/推理/总结/翻译）；多步骤编排顺序；按需加载 references/ 和 templates/ | 全流程脚本化；将多个 scripts/ 串联为长链而不嵌入 Agent 决策节点；LLM 调用逻辑写入 scripts/；无条件分支的"加载脚本→输出" |
...
**量化标准：** SKILL.md 超过 200 行且大部分内容不包含工作流指令时，触发此反模式。
...
| L7 | SKILL.md 体积合理（超过 200 行且不含工作流指令时触发） | 超过 200 行且不含工作流指令时触发 | 【强制】 |
